...
const ensureProductExists = async (productId, tenantId) => {
...
router.get("/", async (req, res, next) => {
  try {
    const entries = await StockEntry.find({ tenant: req.tenant._id })
...
router.post("/entries", async (req, res, next) => {
  try {
    ...
    if (!productId || !(await ensureProductExists(productId, req.tenant._id))) {
